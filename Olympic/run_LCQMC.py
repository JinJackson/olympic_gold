...
from dataset.dataset_LCQMC import TrainDataBert, EvalDataBert
...
def train(model, tokenizer, checkpoint):
    ...
    for epoch in range(checkpoint, args.epochs):
        ...
        # eval dev
        eval_loss, dev_acc= test(model, tokenizer,test_file=args.dev_file, checkpoint=epoch, output_dir=output_dir)
        logger.info(
            f'【DEV】 Train Epoch {epoch}: train_loss={round(np.array(eval_loss).mean(), 4)}, acc:{dev_acc}')
